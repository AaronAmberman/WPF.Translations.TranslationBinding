...
namespace TranslationBindingTesting.Converters
{
    /// <summary>Converter that can be used to calculate if things are equal.</summary>
            ...
            try
            {
                ...
                for (int i = 0; i < values.Length; i++)
                {
                    ...
                    if (targetType == typeof(string))
                    {
                        ...
                        if (val == par) valuesCollection[i] = true;
                        else valuesCollection[i] = false;
                    }
                    ...
                    {
                        if (value == null && parameter == null) valuesCollection[i] = true;
                        else if (value == null && parameter != null) valuesCollection[i] = false;
                        else if (value != null && parameter == null) valuesCollection[i] = false;
                        else valuesCollection[i] = value.Equals(parameter);
                    }
                }

                return valuesCollection.All(x => x); // all value must be true
            }
            catch (Exception ex)
            {
